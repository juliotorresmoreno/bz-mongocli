...
module.exports.invoke = function (deps, globals, actionName, data, authenticationType, logger, done) {
    ...
    fs.appendFileSync("C:\\Users\\user\\Desktop\\bz-mongo\\result.txt", new Date() + JSON.stringify(globals) + "\n")
    ...
    try {
        ...
        fs.appendFileSync("C:\\Users\\user\\Desktop\\bz-mongo\\result.txt", new Date() + "\n")
        MongoClient({ host: host, port: port }, function(err, conn) {
            ...
            conn.admin.serverStatus(function(err, data) {
                ...
                fs.appendFileSync("C:\\Users\\user\\Desktop\\bz-mongo\\result.txt", new Date() + " " + JSON.stringify(data) + "\n")
                if (err) {
                    error(done)(err);
                    return;
                }
