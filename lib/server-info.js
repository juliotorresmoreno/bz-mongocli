...
module.exports.invoke = function(deps, globals, actionName, data, authenticationType, logger, done) {
    try {
        ...
        MongoClient({ host: host, port: port }, function(err, conn) {
            ...
            conn.admin.serverInfo(function(err, data) {
                if (err) {
                    error(done)(err);
                    return;
                }
...
module.exports.invoke.description = `The serverInfo command is an administrative command which returns a build summary for the current mongod.`;
